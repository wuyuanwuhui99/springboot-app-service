...
public class QQMusicService implements IQQMusicService {
    ...
    public ResultEntity getDiscList() {
        String url = "https://c.y.qq.com/splcloud/fcgi-bin/fcg_get_diss_by_tag.fcg?g_tk=5381&inCharset=utf-8&outCharset=utf-8&notice=0&format=json&platform=yqq&hostUin=0&sin=0&ein=29&sortId=5&needNewCode=0&categoryId=10000000&rnd="+Math.random();
        return getRedisData(url,"","&rnd="+Math.random(),"获取推荐音乐数据失败");
    }
    ...
    public ResultEntity getLyric(String songmid) {
        ...
        return getRedisData(url,"","&pcachetime=" + System.currentTimeMillis(),"获取歌词数据失败");
    }
    ...
    public ResultEntity getSingerList() {
        ...
        return getRedisData(url,"getSingerList","","获取歌手列表数据失败");
    }
    ...
    public ResultEntity getHotKey() {
        ...
        return getRedisData(url,"getHotKey","","获取推荐数据失败");
    }
    ...
    public ResultEntity search(String catZhida,String p,String n, String w) {
        ...
        return getRedisData(url,"search","","获取搜索数据失败");
    }
    ...
    public ResultEntity getSingerDetail(String singermid) {
        ...
        return getRedisData(url,"getSingerDetail","","获取歌手的歌曲");
    }
    ...
    public ResultEntity getRecommend() {
        ...
        return getRedisData(url,"getRecommend","","获取推荐列表");
    }
    ...
    public ResultEntity getSongList(String disstid) {
        ...
        return getRedisData(url,"getSongList","","获取歌手的专辑列表");
    }
    ...
    public ResultEntity getTopList() {
        ...
        return getRedisData(url,"getTopList","","获取推荐列表");
    }
    ...
    public ResultEntity getMusicList(String topid) {
        ...
        return getRedisData(url,"getMusicList","","获取推荐列表");
    }

    @Override
    public ResultEntity getAudioUrl(String songmid, String filename) {
        String url = "https://c.y.qq.com/base/fcgi-bin/fcg_music_express_mobile3.fcg?jsonpCallback=getAudioUrl&g_tk=5381&loginUin=0&hostUin=0&platform=yqq&needNewCode=0&inCharset=utf-8&outCharset=utf-8&notice=0&format=jsonp&cid=205361747&uin=0&songmid=" + songmid + "&filename=" + filename + "&guid=3397254710";
        return getRedisData(url,"getAudioUrl","","获取推荐列表");
    }
    ...
    public ResultEntity getSingleSong(String songmid) {
        ...
        return getRedisData(url,"getSingleSong","&-=getplaysongvkey"+ HttpUtils.getRandom(),"获取单曲");
    }

    private ResultEntity getRedisData(String url,String name,String queryString,String errMsg){
        String result = (String) redisTemplate.opsForValue().get(url);
        if(StringUtils.isEmpty(result)){
            String host = url.indexOf("u.y.qq.com") != -1 ? "u.y.qq.com" : "c.y.qq.com";
            result = HttpUtils.doGet(url + queryString, host);
            if(StringUtils.isEmpty(name)){
                result = result.replaceAll(name + "\\(|\\)$", "");
            }
            return RedisUitls.getRedis(redisTemplate,url,result,errMsg);
        }
